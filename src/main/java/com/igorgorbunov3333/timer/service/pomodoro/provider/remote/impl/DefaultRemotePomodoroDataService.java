package com.igorgorbunov3333.timer.service.pomodoro.provider.remote.impl;
...
import com.igorgorbunov3333.timer.service.google.RateLimitedGoogleApiExecutor;
...
@Service
@AllArgsConstructor
public class DefaultRemotePomodoroDataService implements RemotePomodoroDataService {
    ...
    @Override
    @SneakyThrows
    public PomodoroMetadataDto provideRemoteData() {
        String fileId = googleServicesProperties.getDocumentId();
        OutputStream outputStream = new ByteArrayOutputStream();

        rateLimitedGoogleApiExecutor.executeMediaAndDownloadTo(service, fileId, outputStream);

        String json = outputStream.toString();
        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.registerModule(new JavaTimeModule());
        return objectMapper.readValue(json, PomodoroMetadataDto.class);
    }

    @Override
    @SneakyThrows
    public void updateRemoteData(PomodoroMetadataDto pomodoroData) {
        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.registerModule(new JavaTimeModule());
        ...
        Drive.Files.Update updateRequest = service.files()
                .update(documentId, googleDocFile, mediaContent);
        rateLimitedGoogleApiExecutor.execute(updateRequest);
